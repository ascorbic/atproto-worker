...
- ✅ **Firehose** (Phase 4) - WebSocket subscribeRepos event stream
  ...
  - DAG-CBOR frame encoding (header + body concatenation using `@atproto/lex-cbor`)
  - Event broadcasting to connected clients
  - Cursor-based backfill and validation
  - SQLite `firehose_events` table for event persistence
  - **WebSocket/RPC Integration**: Uses `stub.fetch()` instead of RPC methods to avoid WebSocket serialization errors (RPC cannot serialize WebSocket objects per workerd limitation)
  - **Frame Decoding**: Uses `decodeAll()` from `@atproto/lex-cbor` to handle concatenated CBOR objects
  - **CORS Handling**: Standard CORS middleware works with all Hono routes; WebSocket endpoint bypasses CORS naturally
...
export class AccountDurableObject extends DurableObject {
	...
	async createRecord(collection: string, rkey: string, record: unknown) {
		...
		return {
			uri: `at://${this.env.DID}/${collection}/${rkey}`,
			cid: commit.cid,
		};
	}
...
describe("XRPC Endpoints", () => {
	...
	describe("com.atproto.repo.getRecord", () => {
		it("retrieves created record", async () => {
			...
			await SELF.fetch("https://pds.test/xrpc/com.atproto.repo.createRecord", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: "Bearer test-token",
