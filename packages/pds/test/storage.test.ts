...
async function createCid(data: unknown): Promise<{ cid: CID; bytes: Uint8Array }> {
	...
	describe("basic operations", () => {
		...
		it("retrieves multiple blocks with getBlocks()", async () => {
			...
			await runInDurableObject(stub, async (instance: AccountDurableObject) => {
				...
				const result = await storage.getBlocks([block1.cid, block2.cid, nonexistent.cid])
...
describe("AccountDurableObject", () => {
	it("responds to health check", async () => {
		...
		const response = await stub.fetch("https://pds.test/health")
	...
	it("returns 404 for unknown paths", async () => {
		...
		const response = await stub.fetch("https://pds.test/unknown")
